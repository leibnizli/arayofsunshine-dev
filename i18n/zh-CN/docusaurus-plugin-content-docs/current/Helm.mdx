---
sidebar_label: Helm
title: macOS hosts文件管理App | Helm
sidebar_position: 1
---

# Helm

Helm是一个运行在macOS系统的`hosts`文件管理器，Helm使你能够编辑`hosts`文件并在它们之间切换，每次切换的时候Helm会将公共文件（Original File）和激活的文件合并为一个文件写入到mac系统的`hosts`文件。
