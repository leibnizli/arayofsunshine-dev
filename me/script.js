...
[].slice.call(document.querySelectorAll(".scene")).forEach(function(el, i) {
...
(function($,window,undefined){
    ...
    $.iPage.prototype = {
        ...
        _initEvents: function() {
            var self = this,
                start, delta, ishorizontal, el = container;
            var events = {
                ...
                start: function(event) {
                    ...
                    delta={x:0,y:0};
                    removeClass(container,"isanimation");
                ...
                move: function(event) {
                    ...
                    // 不是水平滑动
                    if (!ishorizontal) {
                        container.style.transform = 'translate3d(0px, '+ (-(self.current-1)*pageHeight+delta.y) +'px, 0px)'
                    }
                    ...
                    addClass(container,"isanimation");
                    ...
                    if (Math.abs(delta.y)<=pageWidth / 6) {
                        container.style.transform = 'translate3d(0px, '+ (-(self.current-1)*pageHeight) +'px, 0px)';
                    }
                }
            }
...
if (/\?dev/.test(location.href)) {
    ...
    var loadingNumber = document.getElementById("loading-text");
    ...
    Loader.onFileLoad = function(e) {
        ...
        var percentage = ((Loader.loadCount / Loader.maxLoadCount) * 100).toFixed(0);
        loadingNumber.innerHTML = percentage + '%';
    };
    Loader.onComplete = function(e) {
        loadingNumber = kite = null;
    ...
    function loaderComplete() {
        ...
        $container.iPage({
            onChange:function(i){
                //scenebook.style.display = i==1?"none":"block";
            }
        });
        //$container.iPage("jump",1);
        (function() {
            var $meteor = $("#meteor");
            var $star = $("#star");
            if ($meteor && $star) {
                ...
                $meteor.html(templateHtml);
                $star.html(templateHtml);

                //显示流星

                function showMeteor() {
                    $("#meteor").find("i").each(function(index, element) {
                        var randomNum = Math.random();
                        $(element).css({
                            '-webkit-transform': 'scale(' + randomNum + ')',
                            '-moz-transform': 'scale(' + randomNum + ')',
                    ...
                    $meteor.find("i").addClass("action");
                    //$meteor.find("i").eq(Math.floor(Math.random() * len)).addClass("action");
                    setTimeout(showMeteor, 3000)
                }
                setTimeout(showMeteor, 6000);
                $("#star").find("i").each(function(index, element) {
                    $(element).css({
                        "left": Math.random() * pageWidth,
                        "top": Math.random() * pageHeight,
                        "z-index": index
                ...
                $star.find("i").addClass("action");
            }
        })();
        $body.append('<footer class="footer">如需查看更多信息，在电脑上打开此网址</footer>')
    }
}
